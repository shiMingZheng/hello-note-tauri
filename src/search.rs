...
use tantivy::schema::{Field, IndexRecordOption, Schema, TextFieldIndexing, TextOptions, Value, STRING, STORED, TEXT};
use tantivy::tokenizer::{LowerCaser, RemoveLongFilter, SimpleTokenizer, TextAnalyzer, Tokenizer};
...
pub fn initialize_index(base_path: &Path) -> Result<Arc<Index>> {
    ...
    // 注册 Jieba 分词器
    let tokenizer = JiebaTokenizer::new(JIEBA.clone());
...
fn index_directory_recursive(
    ...
    for entry in entries {
        ...
        if metadata.is_dir() {
            // 递归处理子目录
            index_directory_recursive(writer, fields, &path, base_path, count)?;
