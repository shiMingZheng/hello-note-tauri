...
#[command]
pub async fn is_pinned(relative_path: String, state: State<'_, AppState>) -> Result<bool, String> {
    let db_pool = state.db_pool.lock().unwrap();
    let conn = db_pool.as_ref().ok_or("数据库未初始化")?.get().map_err(|e| e.to_string())?;
    
    let result: i32 = conn.query_row(
        "SELECT COALESCE(is_pinned, 0) FROM files WHERE path = ?1",
...
pub async fn favorite_note(relative_path: String, state: State<'_, AppState>) -> Result<(), String> {
	let db_pool = state.db_pool.lock().unwrap();
	let conn = db_pool.as_ref().ok_or("数据库未初始化")?.get().map_err(|e| e.to_string())?;
	conn.execute("UPDATE files SET is_favorited = 1 WHERE path = ?1", params![relative_path])
	.map_err(|e| e.to_string())?;
	Ok(())
}
#[command]
pub async fn unfavorite_note(relative_path: String, state: State<'_, AppState>) -> Result<(), String> {
	let db_pool = state.db_pool.lock().unwrap();
	let conn = db_pool.as_ref().ok_or("数据库未初始化")?.get().map_err(|e| e.to_string())?;
	conn.execute("UPDATE files SET is_favorited = 0 WHERE path = ?1", params![relative_path])
	.map_err(|e| e.to_string())?;
	Ok(())
}
#[command]
pub async fn get_favorited_notes(state: State<'_, AppState>) -> Result<Vec<PinnedNote>, String> {
	let db_pool = state.db_pool.lock().unwrap();
	let conn = db_pool.as_ref().ok_or("数据库未初始化")?.get().map_err(|e| e.to_string())?;
	let mut stmt = conn.prepare("SELECT path, title FROM files WHERE is_favorited = 1 ORDER BY title")
		.map_err(|e| e.to_string())?;
...
#[command]
pub async fn is_favorited(relative_path: String, state: State<'_, AppState>) -> Result<bool, String> {
	let db_pool = state.db_pool.lock().unwrap();
	let conn = db_pool.as_ref().ok_or("数据库未初始化")?.get().map_err(|e| e.to_string())?;
	let result: i32 = conn.query_row(
		"SELECT COALESCE(is_favorited, 0) FROM files WHERE path = ?1",
		params![relative_path],
		|row| row.get(0)
	).unwrap_or(0);
