...
pub async fn pin_note(path: String, state: State<'_, AppState>) -> Result<(), String> {
    ...
    conn.execute("UPDATE files SET is_pinned = 1 WHERE path = ?1", params![path])
...
pub async fn unpin_note(path: String, state: State<'_, AppState>) -> Result<(), String> {
    let db_pool = state.db_pool.lock().unwrap();
    let conn = db_pool.as_ref().ok_or("数据库未初始化")?.get().map_err(|e| e.to_string())?;
    conn.execute("UPDATE files SET is_pinned = 0 WHERE path = ?1", params![path])
        .map_err(|e| e.to_string())?;
    Ok(())
}
...
pub async fn get_pinned_notes(state: State<'_, AppState>) -> Result<Vec<PinnedNote>, String> {
    ...
    let mut stmt = conn.prepare(
        "SELECT path, title FROM files WHERE is_pinned = 1 ORDER BY title"
    ).map_err(|e| e.to_string())?;
